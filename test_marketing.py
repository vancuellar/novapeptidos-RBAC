...
def test_cac_es_gasto_entre_clientes_nuevos():
    ...
    r = m.cruzar(filas, pedidos, fx=18.0)
    fila = r['campanas'][0]
    assert fila['gasto_mxn'] == 1800          # 100 USD x 18
    assert fila['clientes_nuevos'] == 2
    assert fila['cac'] == 900.0               # 1800 / 2
    assert fila['roas'] == round(5000 / 1800, 2)
...
def test_un_cliente_que_repite_NO_abarata_el_costo():
    ...
    filas = [{'campaign': 'Verano', 'spend': 100.0, 'currency': 'USD', 'link_clicks': 200}]
    solo_nuevo = m.cruzar(filas, [_pedido('Verano', 3000)], fx=18.0)['campanas'][0]
    con_repetidor = m.cruzar(filas, [_pedido('Verano', 3000),
                                     _pedido('Verano', 9000, nuevo=False)], fx=18.0)['campanas'][0]
    assert solo_nuevo['cac'] == con_repetidor['cac'] == 1800.0
    assert con_repetidor['pedidos'] == 2          # los dos pedidos sí se ven
    assert con_repetidor['ingreso'] == 12000      # y su ingreso también
...
def test_sin_clientes_el_cac_es_nulo_no_cero():
    ...
    fila = m.cruzar(filas, [], fx=18.0)['campanas'][0]
...
def test_lo_sin_etiquetar_va_aparte_y_no_se_reparte():
    ...
    assert r['sin_etiquetar']['ingreso'] == 5000
...
def test_campana_apagada_que_sigue_vendiendo_no_desaparece():
    ...
    r = m.cruzar([], [_pedido('Vieja', 4000)], fx=18.0)
    fila = next(f for f in r['campanas'] if f['slug'] == 'vieja')
    assert fila['gasto_mxn'] == 0 and fila['ingreso'] == 4000


def test_gasto_en_pesos_no_se_multiplica_por_el_tipo_de_cambio():
    filas = [{'campaign': 'MX', 'spend': 1800.0, 'currency': 'MXN', 'link_clicks': 200}]
    assert m.cruzar(filas, [], fx=18.0)['campanas'][0]['gasto_mxn'] == 1800


def test_con_poco_gasto_dice_sin_datos_en_vez_de_juzgar():
    filas = [{'campaign': 'Nueva', 'spend': 1.0, 'currency': 'USD', 'link_clicks': 3}]
    assert m.cruzar(filas, [], fx=18.0)['campanas'][0]['veredicto'] == 'sin datos'
...
def test_el_canal_directo_no_finge_tener_costo():
    ...
    r = m.canales([_p(3000)], gasto_meta_mxn=1800)
...
def test_meta_si_lleva_su_costo_y_su_cac():
    r = m.canales([_p(3000, {'fbclid': 'x'}), _p(5000, {'fbclid': 'y'})], gasto_meta_mxn=1800)
    meta = next(f for f in r['por_origen'] if f['canal'] == 'meta')
    assert meta['clientes_nuevos'] == 2 and meta['costo'] == 1800
    assert meta['cac'] == 900.0
...
def test_el_costo_del_distribuidor_es_su_comision():
    ...
    assert d['comisiones'] == 3200
    assert d['cac'] == 3200.0            # 3200 de comision / 1 cliente nuevo
...
def test_el_traslape_meta_distribuidor_se_dice_en_voz_alta():
    ...
    r = m.canales(pedidos, gasto_meta_mxn=1800)
